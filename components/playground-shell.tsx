...
export function PlaygroundShell() {
  ...
  const localizedLessons = useMemo(() => getPlaygroundLessonsForLocale(locale), [locale]);
  ...
  const initialId = searchParams.get('lesson') ?? localizedLessons[0].id;
  const initialLesson = localizedLessons.find((l) => l.id === initialId) ?? localizedLessons[0];
  ...
  const [activeFrames, setActiveFrames] = useState<ExecutionFrame[]>(initialLesson.executionFrames);
  const [runOutput, setRunOutput] = useState<string[]>([]);
  const [runError, setRunError] = useState<string | null>(null);
  const [hasRun, setHasRun] = useState(false);
  const [activeTab, setActiveTab] = useState<'explain' | 'complexity'>('explain');
  const [explanationMode, setExplanationMode] = useState<'simple' | 'deepDive'>('simple');
  ...
  const grouped = levelOrder
    .map((lvl) => ({ level: lvl, items: localizedLessons.filter((l) => l.level.toLowerCase() === lvl) }))
    .filter((g) => g.items.length > 0);

  const [expandedLevels, setExpandedLevels] = useState<Set<string>>(() => new Set([lesson.level.toLowerCase()]));
  ...
  // Keep selected lesson in sync with ?lesson= query param.
  useEffect(() => {
    const lessonFromQuery = searchParams.get('lesson');
    if (!lessonFromQuery) return;

    const found = localizedLessons.find((item) => item.id === lessonFromQuery);
    if (found && found.id !== selectedLessonId) {
      setSelectedLessonId(found.id);
    }
  }, [searchParams, localizedLessons, selectedLessonId]);
  ...
  useEffect(() => {
    ...
    if (!searchParams.get('lesson') && lastLessonId && selectedLessonId !== lastLessonId) {
      ...
      setActiveFilter(localizedLessons.find((l) => l.id === lastLessonId)?.level.toLowerCase() ?? activeFilter);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mounted, searchParams, localizedLessons, selectedLessonId, lastLessonId, activeFilter]);
    ...
    <div className="site-shell relative px-3 py-6 sm:py-10 sm:px-10 lg:px-16" dir={isRtlLocale ? 'rtl' : 'ltr'}>
      ...
      <section className="relative z-10 mb-6 card-elevated rounded-[24px] sm:rounded-[32px] p-4 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <h1 className="mt-4 text-2xl font-semibold leading-snug text-bright sm:text-3xl lg:text-4xl">{lesson.title}</h1>
            <p
              className="mt-3 max-w-2xl text-sm leading-7 text-ink/60"
              dangerouslySetInnerHTML={{
                __html: t('subheadline', { runMyCode: `<strong class="font-semibold text-ink">${t('runMyCode')}</strong>` }),
              }}
            />
          </div>
          <div className="flex shrink-0 flex-col gap-2 sm:items-end">
            {[
              ...
              <div key={item.label} className="inline-flex items-center gap-2 rounded-full bg-parchment px-4 py-2 text-sm">
      ...
      <section className="relative z-10 mb-8 rounded-[24px] sm:rounded-[32px] bg-gradient-to-b from-parchment to-mist border border-white/5 p-4 sm:p-8 shadow-xl">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="text-[10px] sm:text-xs font-bold uppercase tracking-wider sm:tracking-[0.25em] text-signal/80 mb-1">{t('dataFlow')}</div>
            <h2 className="text-xl font-bold text-bright capitalize">
              ...
              >
                {activeFilter === 'all' ? lesson.level : activeFilter} {t('track')}
        ...
        <div className="relative">
          ...
          <div className="relative flex items-center justify-between gap-4 overflow-x-auto custom-scrollbar pb-4 pt-2">
            {grouped
              .find((g) => g.level === (activeFilter === 'all' ? lesson.level.toLowerCase() : activeFilter))
              ?.items.map((item, i) => {
                ...
                return (
                  <div key={item.id} className="relative flex flex-col items-center shrink-0 min-w-[120px] group">
                    <button
                      ...
                      className={`relative z-10 flex h-14 w-14 items-center justify-center rounded-full border-4 pipeline-node-bg transition-all duration-300 ${
                        active
                          ...
                          : completed
                            ? 'border-leaf hover:scale-105 shadow-[inset_0_0_12px_rgba(74,222,128,0.2)]'
                    ...
                    <div
                      className={`mt-4 text-center text-xs font-semibold leading-relaxed transition-colors line-clamp-2 ${
      ...
      <section className="relative z-10 grid gap-6 grid-cols-1 lg:grid-cols-[1fr_1.2fr_1fr] xl:grid-cols-[340px_minmax(0,1.2fr)_340px] 2xl:grid-cols-[380px_minmax(0,1.3fr)_380px]">
        ...
        <aside className={`space-y-6 flex flex-col ${isRtlLocale ? 'text-right' : ''}`}>
          {/* Explain block */}
          <motion.div
          ...
          >
            ...
            {lesson.explanation && activeExplanation && (
              <div className="mt-6 space-y-3 rounded-[20px] border border-white/5 bg-white/[0.02] p-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-ink/50">Structured Explanation</div>
                  <div className="flex items-center gap-2">
                    {(['simple', 'deepDive'] as const).map((mode) => (
                      <button
                        key={mode}
                        onClick={() => setExplanationMode(mode)}
                        className={`rounded-full px-3 py-1 text-[11px] font-semibold transition-colors border ${
                          explanationMode === mode ? 'border-signal text-signal bg-signal/10' : 'border-white/10 text-ink/60 hover:text-ink/80'
                        }`}
                      >
                        {mode === 'simple' ? 'Simple' : 'Deep dive'}
                      </button>
                    ))}
                  </div>
                </div>

                <div className="space-y-4 text-xs leading-relaxed text-ink/75">
                  <div>
                    <div className="font-semibold text-ink/90">Overview</div>
                    <p>{activeExplanation.overview}</p>
                  </div>
                  {activeExplanation.analogy && (
                    <div>
                      <div className="font-semibold text-ink/90">Analogy</div>
                      <p>{activeExplanation.analogy}</p>
                    </div>
                  )}
                  <div>
                    <div className="font-semibold text-ink/90">Step-by-step</div>
                    <ul className="list-disc pl-4 space-y-1">
                      {activeExplanation.steps.map((step) => (
                        <li key={step}>{step}</li>
                      ))}
                    </ul>
                  </div>
                  {activeExplanation.visual && (
                    <div>
                      <div className="font-semibold text-ink/90">Visual</div>
                      <p className="font-mono text-[11px] text-ink/80 whitespace-pre-wrap">{activeExplanation.visual}</p>
                    </div>
                  )}
                  <div>
                    <div className="font-semibold text-ink/90">Common mistakes</div>
                    <ul className="list-disc pl-4 space-y-1">
                      {activeExplanation.commonMistakes.map((item) => (
                        <li key={item}>{item}</li>
                      ))}
                    </ul>
                  </div>
                  <div>
                    <div className="font-semibold text-ink/90">Practice questions</div>
                    <ul className="list-disc pl-4 space-y-1">
                      {activeExplanation.practice.map((item) => (
                        <li key={item}>{item}</li>
                      ))}
                    </ul>
                  </div>
                  {(activeExplanation.timeComplexity || activeExplanation.spaceComplexity) && (
                    <div className="grid grid-cols-2 gap-3 text-[11px]">
                      {activeExplanation.timeComplexity && (
                        <div className="rounded-lg border border-white/10 bg-black/20 px-3 py-2">
                          <div className="text-ink/60 uppercase tracking-[0.18em] text-[9px]">Time</div>
                          <div className="font-mono text-ink/90">{activeExplanation.timeComplexity}</div>
                        </div>
                      )}
                      {activeExplanation.spaceComplexity && (
                        <div className="rounded-lg border border-white/10 bg-black/20 px-3 py-2">
                          <div className="text-ink/60 uppercase tracking-[0.18em] text-[9px]">Space</div>
                          <div className="font-mono text-ink/90">{activeExplanation.spaceComplexity}</div>
                        </div>
                      )}
                    </div>
                  )}
        ...
        <section className="space-y-6 flex flex-col">
          <div className="card-elevated rounded-[24px] sm:rounded-[32px] p-4 sm:p-5">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                <div className="text-sm font-semibold uppercase tracking-[0.2em] text-ink/42">{t('codeStudio')}</div>
                <h2 className="mt-2 text-3xl font-semibold text-bright">{lesson.title}</h2>
              </div>
              <div className="flex flex-wrap gap-2">
                <div className="rounded-full bg-mist px-4 py-2 text-sm font-medium text-ink">{t('monacoReady')}</div>
                <div
                  className={`rounded-full px-4 py-2 text-sm font-medium ${
                    pyStatus === 'ready' ? 'bg-leaf/12 text-leaf' : pyStatus === 'error' ? 'bg-signal/12 text-signal' : 'bg-amber/12 text-amber'
                  }`}
                >
                ...
                {activeFrames.map((item, index) => (
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-medium transition ${
                      index === frameIndex ? 'bg-signal text-white' : 'bg-parchment text-ink hover:bg-mist'
                    }`}
                  >
                    {item.event}
                  </button>
          ...
          <div className="card-elevated rounded-[32px] p-6 shadow-md flex-1 overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-6">
              ...
              {(() => {
                ...
                const getStableHeapAddress = (name: string) => {
                  const hash = Array.from(name).reduce((acc, ch) => (acc * 31 + ch.charCodeAt(0)) & 0xffff, 0);
                          ...
                          {heapVars.map((entry) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {safeArrayItems.length > 0 ? (
                                    safeArrayItems.map((item, idx) => (
                                      <motion.div
                                        layout
                                        key={item.key}
      ...
      <section className="relative z-10 mt-6 mb-12 card-elevated rounded-[32px] p-8 shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {lesson.useCases.map((uc, i) => (
            <div
              key={uc}
              className="flex gap-4 items-start border border-white/5 bg-white/[0.02] p-5 rounded-[24px] hover:bg-white/[0.04] transition-colors"
            >

    ...
    <header className="px-4 pt-4 sm:px-6 sm:pt-6 lg:px-16">
      <div className="site-shell flex flex-wrap items-center justify-between gap-3 rounded-[24px] sm:rounded-full border border-white/10 bg-parchment/90 px-4 py-3 sm:px-5 backdrop-blur-md shadow-[0_2px_20px_rgba(0,0,0,0.4)]">
        <Link href="/" className="flex items-center gap-2 shrink-0">
          ...
          <span className="text-gradient-signal text-lg font-bold neon-text-signal ml-1 hidden sm:block">
            PyAnimate
          </span>
        </Link>
        <div className="flex flex-wrap items-center gap-3 sm:gap-6">
          {compactLabel ? (
            <div className="text-xs sm:text-sm font-medium text-ink/72 max-w-[140px] sm:max-w-none truncate sm:whitespace-normal" title={compactLabel}>
              {compactLabel}
            </div>
          ) : (
            <nav className="flex flex-wrap items-center gap-3 sm:gap-5 text-xs sm:text-sm font-medium text-ink/65">
              <a href="#tracks" className="hidden md:block transition hover:text-wave hover:neon-text-wave">{t("tracks")}</a>
              <a href="#algorithms" className="hidden md:block transition hover:text-wave">{t("algorithms")}</a>
              <Link
                href="/playground"

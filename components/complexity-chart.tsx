...
export function ComplexityChart({ timeComplexity, spaceComplexity }: Props) {
  ...
  // grid lines
  const gridY = [0.25, 0.5, 0.75, 1.0].map(
    (t) => PAD.top + PLOT_H - t * PLOT_H,
  );
  const gridX = [0.25, 0.5, 0.75, 1.0].map(
    (t) => PAD.left + t * PLOT_W,
  );
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-[16px] overflow-hidden bg-[#0f172a] p-1">
        <svg
          ...
          width="100%"
          className="block"
          aria-label="Big O complexity curves"
        >
          {/* Grid lines */}
          {gridY.map((y) => (
            <line
        ...
        {highlighted.map((c) => {
          ...
          return (
            ...
            >
              ...
              {c.activeSpace && (
                <span className="text-white/40 text-[9px]">space</span>
              )}
            </div>
          );
        })}
